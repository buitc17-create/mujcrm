...
export function buildOutreachEmailHtml(jmeno: string | null, unsubscribeToken: string): string {
...
export async function sendOutreachEmail(to: string, jmeno: string | null, unsubscribeToken: string): Promise<SendResult> {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: 'CRM ušitý na míru realitním makléřům — 7 dní zdarma',
        html: buildOutreachEmailHtml(jmeno, unsubscribeToken),
      }),
    })
