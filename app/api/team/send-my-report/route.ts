...
// Člen volá tuto route sám — pošle svůj výkaz adminovi
export async function POST() {
  ...
  if (!tm) {
    return NextResponse.json({ error: 'Nejste přiřazen k žádnému týmu.' }, { status: 403 });
  }
  ...
  try {
    ...
    if (!emailSettings) {
      return NextResponse.json(
        { error: 'Váš nadřízený nemá nastavený odchozí e-mail — výkaz nelze odeslat.' },
        { status: 422 }
      );
    }

    // Info o adminovi a o přihlášeném členu
    const [{ data: adminAuth }, { data: memberAuth }] = await Promise.all([
      admin.auth.admin.getUserById(tm.owner_id),
    ...
    const memberName  = memberAuth.user?.user_metadata?.full_name
      ?? memberEmail.split('@')[0]
      ?? 'Člen týmu';
    if (!adminEmail) return NextResponse.json({ error: 'Nepodařilo se načíst e-mail nadřízeného.' }, { status: 500 });
